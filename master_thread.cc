...
bool CMasterThread::CheckLibeventVersion()
{
	...
	syslog(LOG_INFO, "The libevent version is %s" ,libevent_version);
...
bool CMasterThread::InitMasterThread()
{
	...
	{
		syslog(LOG_INFO, "CMasterThread::InitMasterThread:The libevent version Require at  2.0.*");
		return false;
	}
	...
	if(event_add(listen_event_, NULL) == -1)
	{
		...
		syslog(LOG_INFO, "CMasterThread::InitMasterThread:event_add errorCode = %d, description = %s" ,error_code
...
void CMasterThread::Run()
{
	...
	syslog(LOG_INFO, "CMasterThread::Run:Master thread has start...");

	int ret = event_base_dispatch(main_base_);
	if (-1 == ret)
	{
		int error_code = EVUTIL_SOCKET_ERROR();
		syslog(LOG_INFO, "CMasterThread::Run():event_base_dispatch errorCode = %d, description = %s" ,error_code
					  ,evutil_socket_error_to_string(error_code));
		exit(1);
	}
	else if(1 == ret)
	{
		syslog(LOG_INFO, "CMasterThread::Run():no events were registered.");
		exit(1);
	}
}
...
void CMasterThread::AccepCb(evutil_socket_t listen_socket, short event, void* arg)
{
	...
	if (-1 == sfd)
	{
		syslog(LOG_INFO, "CMasterThread::AccepCb:accept error = %s" ,strerror(errno));
		return;
	}
	...
	{
		syslog(LOG_INFO, "CMasterThread::AccepCb:SetSocketNoBlock error = %s" ,strerror(errno));
		close(sfd);
		return;
	}
...
bool CMasterThread::InitRemoteListenSocket(evutil_socket_t& listen_socket)
{
	...
	if (listen_socket < 0)
	{
		syslog(LOG_INFO, "CMasterThread::InitRemoteListenSocket:socket error = %s" ,strerror(errno));
		return false;
	}
	...
	{
		syslog(LOG_INFO, "CMasterThread::InitRemoteListenSocket:setsockopt SO_REUSEADDR error = %s" ,strerror(errno));
		close(listen_socket);
		return false;
	}
	...
	{
		syslog(LOG_INFO, "CMasterThread::InitRemoteListenSocket:setsockopt TCP_NODELAY error = %s" ,strerror(errno));
		close(listen_socket);
		return false;
	}
	...
	{
		syslog(LOG_INFO, "CMasterThread::InitRemoteListenSocket:bind error = %s" ,strerror(errno));
		close(listen_socket);
		return false;
	}
	...
	{
	    syslog(LOG_INFO, "CMasterThread::InitRemoteListenSocket:Listen error = %s" ,strerror(errno));
	    close(listen_socket);
	    return false;
	}
